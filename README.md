...
- **SQL**: Cleaned data is also loaded into a local SQLite database (`job_market.db`, generated by the notebook) and queried directly with SQL (see "SQL Analysis" section in the notebook), cross-validating the pandas-based findings from Tasks 2 and 4.
- **Neural Network**: TensorFlow is not yet compatible with Python 3.14 (this project's environment). As a substitute, scikit-learn's `MLPRegressor` (same foundational Multi-Layer Perceptron architecture) was used as an alternative model for Task 3, compared directly against the linear regression baseline (see "Neural Network Comparison" section in the notebook).